WAAG.Map = function Map(domains) {
			...
			layer.sdkData.forEach(function(d) {
				...
				if (d.geom) {
					...
					if (layer.sdkPath == "mainMap") {
						...
						d.value = d.timestamp;
					}else if(layer.id=="artsholland"){
						d.value = 0.5;
						if(d.layers.artsholland.data.events){
							d.layers.artsholland.data.events.forEach(function(e){
								d.value+=0.05;
							});
	...
	updatePolygonMap = function(layer) {
		...
		var vis = visMap.selectAll("path").data(data, function(d, i) {
			return d.cdk_id
		});
	...
	updatePointMap = function(layer) {
		...
		var vis = visPointMap.selectAll("path").data(data, function(d, i) {
			return d.cdk_id
		});
	...
	updateLabelsMap = function(layer) {
		...
		var labels = visPointMap.selectAll("use").data(data, function(d, i) {
			return d.cdk_id
		});
	...
	function setMapToolTipLabel(_data, _path) {
		...
		var v = _data;
		_path.split(":").forEach(function(d) {
			v = v[d];
		});
