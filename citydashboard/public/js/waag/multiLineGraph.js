WAAG.MultiLineGraph = function MultiLineGraph(properties, _subDomain) {
	...
	function init() {
		...
		color.domain(d3.keys(dataNow[0].value).filter(function(key) {
			return key
		}));
		...
		var max;
		var max1 = d3.max(subjectsNow, function(c) {
			return d3.max(c.values, function(v) {
				return v.value;
			});
		})
		var max2 = d3.max(subjectsHistory, function(c) {
			return d3.max(c.values, function(v) {
				return v.value;
			});
		});
		...
		legend.selectAll('g').data(subjectsNow, function(d, i) {
			return i;
		})
			.enter()
			.append('g')
	...
	function updateGraph(dataNow, dataHistory, data) {
		...
		var visLineNow = svgDomain.selectAll("path.lineNow").data(dataNow, function(d, i) {
			return i;
		});
		...
		var visLineHistory = svgDomain.selectAll("path.lineHistory").data(dataHistory, function(d, i) {
			return i;
		});
